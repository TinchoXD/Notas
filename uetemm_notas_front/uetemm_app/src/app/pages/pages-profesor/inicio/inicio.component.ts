...
export class InicioComponent implements OnInit {
  ...
  cursosProfesor: any[] = [];
  asignaturasPorCurso!: Record<
    string,
    { nombreAsignatura: string; cursoProfesor_id: number; dataNotas: any }[]
  >;
  ...
  private agruparAsignaturasPorCurso(
    ...
    { nombreAsignatura: string; cursoProfesor_id: number; dataNotas: any }[]
  > {
    const asignaturasPorCurso: Record<
      string,
      { nombreAsignatura: string; cursoProfesor_id: number; dataNotas: any }[]
    > = {};
  
    cursosProfesor.forEach((cursoProfesor, index) => {
      const nombreCurso =
      ...
      this.notaService.getNotasByCursoProfesor(cursoProfesor.id).subscribe({
        next: (notas) => {
            ...
            datasets: [
              {
                ...
                data: [maxCalificacionT1, maxCalificacionT2, maxCalificacionT3],
                fill: false,
              ...
              },
              {
                ...
                data: [minCalificacionT1, minCalificacionT2, minCalificacionT3],
                fill: false,
          ...
          // Agregar el curso con sus datos de notas ya procesados
          const value = { nombreAsignatura, cursoProfesor_id, dataNotas };
  ...
  /* loadAllData() {
    ...
    this.cursosProfesor.forEach((cursoProfesor, index) => {
      this.notaService.getNotasByCursoProfesor(cursoProfesor.id).subscribe({
        next: (notas) => {
          const asignatura = cursoProfesor.asignatura.nombre;
