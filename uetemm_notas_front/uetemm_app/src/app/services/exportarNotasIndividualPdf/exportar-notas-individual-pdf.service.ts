...
export class ExportarNotasIndividualPdfService {
  ...
  async exportarPDF(estudiante: any): Promise<void> {
    ...
    this.estudianteService.getEstudianteById(this.estudiante?.id).subscribe({
      next: (estudiante) => {
        if (estudiante) {
          this.estudiante = estudiante;
                ...
                .subscribe({
                  next: async (notaComportamiento) => {
                    ...
                    contenidoTabla.push([
                      ...
                      {}, // Celdas vacías combinadas
                      {
                        text: this.convertirCulitativo(
                          await this.calcularPromedioAnual()
                        ),
                        colSpan: 1,
                        style: 'tableBody',
                      },
                    ...
                    const docDefinition: any = {
                      content: [
                        {
                          text: 'UNIDAD EDUCATIVA MITAD DEL MUNDO',
                          style: 'header',
                        },
