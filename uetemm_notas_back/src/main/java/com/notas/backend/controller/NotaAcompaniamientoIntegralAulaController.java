...
@RestController
@RequestMapping("/api/notas/notaAcompaniamientoIntegralAula")
@RequiredArgsConstructor
@CrossOrigin(origins = { "*" })
/* @CrossOrigin(origins = { "http://localhost:4600" }) */
public class NotaAcompaniamientoIntegralAulaController {
    ...
    */
    ...
    public ResponseEntity<Object> getNotaAcompaniamientoIntegralByEstudianteIdAndCursoId(@PathVariable int estu_id,
            @PathVariable int curs_id) {
        try { 
            NotaAcompaniamientoIntegralAula notaAcompaniamientoIntegralAula = notaService.getNotaAcompaniamientoIntegralByEstudianteIdAndCursoId(estu_id, curs_id);
            return ResponseEntity.ok(notaAcompaniamientoIntegralAula);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(
