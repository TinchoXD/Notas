...
@RestController
@RequestMapping("/api/notas/notaComportamiento")
@RequiredArgsConstructor
@CrossOrigin(origins = { "*" })
/* @CrossOrigin(origins = { "http://localhost:4600" }) */
public class NotaComportamientoController {
    ...
    */
    ...
    public ResponseEntity<Object> getNotaComportamientoByEstudianteIdAndCursoId(@PathVariable int estu_id,
            @PathVariable int curs_id) {
        try { 
            NotaComportamiento notaComportamiento = notaService.getNotaComportamientoByEstudianteIdAndCursoId(estu_id, curs_id);
            return ResponseEntity.ok(notaComportamiento);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @PostMapping(value = "/nota/guardarNotaComportamiento")
    public ResponseEntity<Object> postNota(@RequestBody NotaComplementariaRequest notaRequest) {
        
        
        return ResponseEntity.ok(notaService.postNotaComportamiento(notaRequest));
    }
